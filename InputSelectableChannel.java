...
public class InputSelectableChannel {
    ...
    private final Pipe pipe;  //a pair write and read pipes  sink -- write   source -- read

    private final ChannelThread channel;

    public InputSelectableChannel(InputStream input) throws IOException {
        this.pipe = Pipe.open();
        this.channel = new ChannelThread(input, pipe.sink()); //input stream -> buffer -> write to channel
    }
